...
public class BLAS {
	...
	/**
	 * y += a * x .
	 * Note: for sparseVector dot denseVector, we allow size sparse vector with size as -1
	 */
	public static void axpy(double a, SparseVector x, DenseVector y) {
		if(x.size() == -1) {
			int lastValidId = x.indices.length - 1;
			while (x.indices[lastValidId] >= y.size()){
				lastValidId --;
			}
			for (int i = 0; i <= lastValidId; i++) {
	...
	/**
	 * y := alpha * A * x + beta * y .
	 * Note: for sparseVector, we allow size sparse vector with size as -1
	 */
	public static void gemv(double alpha, DenseMatrix matA, boolean transA,
		...
		if (transA) {
			int start = 0;
			int lastValidId = x.indices.length - 1;
			while (x.indices[lastValidId] >= m){
				lastValidId --;
			}
			for (int i = 0; i < n; i++) {
				double s = 0.;
				for (int j = 0; j <= lastValidId; j++) {
