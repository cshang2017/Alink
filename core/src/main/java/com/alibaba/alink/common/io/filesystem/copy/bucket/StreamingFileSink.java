 ...
 * {@link StreamingFileSink#forRowFormat(Path, BaseFileSystem, Encoder)} or
 * {@link StreamingFileSink#forBulkFormat(Path, BaseFileSystem, BulkWriter.Factory)}.
 *
 *
...
public class StreamingFileSink<IN>
	...
	public static <IN> StreamingFileSink.RowFormatBuilder <IN, String> forRowFormat(
		final Path basePath, final BaseFileSystem <?> fileSystem, final Encoder <IN> encoder) {
		return new StreamingFileSink.RowFormatBuilder <>(basePath, fileSystem, encoder,
	...
	public static <IN> StreamingFileSink.BulkFormatBuilder <IN, String> forBulkFormat(
		final Path basePath, final BaseFileSystem <?> fileSystem, final BulkWriter.Factory <IN> writerFactory) {
		return new StreamingFileSink.BulkFormatBuilder <>(basePath, fileSystem, writerFactory,
			new DateTimeBucketAssigner <>());
	}
	...
	public static class RowFormatBuilder<IN, BucketID> extends StreamingFileSink.BucketsBuilder <IN, BucketID> {
		...
		RowFormatBuilder(Path basePath, BaseFileSystem <?> fileSystem, Encoder <IN> encoder,
						 BucketAssigner <IN, BucketID> bucketAssigner) {
			this(basePath, fileSystem, encoder, bucketAssigner, DefaultRollingPolicy.create().build(), 60L * 1000L,
		...
		public StreamingFileSink.RowFormatBuilder <IN, BucketID> withBucketCheckInterval(final long interval) {
			return new RowFormatBuilder <>(basePath, fileSystem, encoder, bucketAssigner, rollingPolicy, interval,
				bucketFactory);
		}

		public StreamingFileSink.RowFormatBuilder <IN, BucketID> withBucketAssigner(
			final BucketAssigner <IN, BucketID> assigner) {
			return new RowFormatBuilder <>(basePath, fileSystem, encoder, Preconditions.checkNotNull(assigner),
				rollingPolicy, bucketCheckInterval, bucketFactory);
		}

		public StreamingFileSink.RowFormatBuilder <IN, BucketID> withRollingPolicy(
			final RollingPolicy <IN, BucketID> policy) {
			return new RowFormatBuilder <>(basePath, fileSystem, encoder, bucketAssigner,
				Preconditions.checkNotNull(policy), bucketCheckInterval, bucketFactory);
		}
		...
		public <ID> StreamingFileSink.RowFormatBuilder <IN, ID> withBucketAssignerAndPolicy(
			...
			return new RowFormatBuilder <>(basePath, fileSystem, encoder, Preconditions.checkNotNull(assigner),
				Preconditions.checkNotNull(policy), bucketCheckInterval, new DefaultBucketFactoryImpl <>());
		}
		...
		@VisibleForTesting
		StreamingFileSink.RowFormatBuilder <IN, BucketID> withBucketFactory(
			final BucketFactory <IN, BucketID> factory) {
			return new RowFormatBuilder <>(basePath, fileSystem, encoder, bucketAssigner, rollingPolicy,
				bucketCheckInterval, Preconditions.checkNotNull(factory));
		}
	}
	...
	public static class BulkFormatBuilder<IN, BucketID> extends StreamingFileSink.BucketsBuilder <IN, BucketID> {
		...
		BulkFormatBuilder(Path basePath, BaseFileSystem <?> fileSystem, BulkWriter.Factory <IN> writerFactory,
						  BucketAssigner <IN, BucketID> assigner) {
			this(basePath, fileSystem, writerFactory, assigner, 60L * 1000L, new DefaultBucketFactoryImpl <>());
		}
		...
		public StreamingFileSink.BulkFormatBuilder <IN, BucketID> withBucketCheckInterval(long interval) {
			return new BulkFormatBuilder <>(basePath, fileSystem, writerFactory, bucketAssigner, interval,
				bucketFactory);
		}

		public <ID> StreamingFileSink.BulkFormatBuilder <IN, ID> withBucketAssigner(BucketAssigner <IN, ID> assigner) {
			return new BulkFormatBuilder <>(basePath, fileSystem, writerFactory, Preconditions.checkNotNull(assigner),
				bucketCheckInterval, new DefaultBucketFactoryImpl <>());
		}

		@VisibleForTesting
		StreamingFileSink.BulkFormatBuilder <IN, BucketID> withBucketFactory(
			final BucketFactory <IN, BucketID> factory) {
			return new BulkFormatBuilder <>(basePath, fileSystem, writerFactory, bucketAssigner, bucketCheckInterval,
	...
	public void snapshotState(FunctionSnapshotContext context) throws Exception {
		Preconditions.checkState(bucketStates != null && maxPartCountersState != null, "sink has not been "
			+ "initialized");
