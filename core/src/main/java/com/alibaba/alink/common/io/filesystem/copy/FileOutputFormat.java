...
public abstract class FileOutputFormat<IT> extends RichOutputFormat <IT>
	...
	public static void initDefaultsFromConfiguration(Configuration configuration) {
		...
		DEFAULT_OUTPUT_DIRECTORY_MODE = alwaysCreateDirectory ? OutputDirectoryMode.ALWAYS
			: OutputDirectoryMode.PARONLY;
	}
	...
	public FileOutputFormat(BaseFileSystem <?> fs) {this(null, fs);}

	public FileOutputFormat(Path outputPath, BaseFileSystem <?> fs) {
	...
	public void open(int taskNumber, int numTasks) throws IOException {
		...
		if (!fs.isDistributedFS()) {
			...
			if (numTasks == 1 && outputDirectoryMode == OutputDirectoryMode.PARONLY) {
				...
				// prepare local output path. checks for write mode and removes existing files in case of OVERWRITE
				// mode
		...
		this.actualFilePath = (numTasks > 1 || outputDirectoryMode == OutputDirectoryMode.ALWAYS) ? p.suffix(
	...
	public void tryCleanupOnError() {
		if (this.fileCreated) {
			this.fileCreated = false;
				...
				FileSystemUtils.getFlinkFileSystem(fs, this.actualFilePath.toString()).delete(actualFilePath, false);
		}
	}
}
