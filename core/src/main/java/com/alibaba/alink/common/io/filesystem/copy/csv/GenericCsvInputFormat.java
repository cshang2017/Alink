...
public abstract class GenericCsvInputFormat<OT> extends DelimitedInputFormat <OT> {
	...
	protected GenericCsvInputFormat(BaseFileSystem <?> fs) {
		super(fs);
	}

	protected GenericCsvInputFormat(Path filePath, BaseFileSystem <?> fs) {
		super(filePath, null, fs);
	}
	...
	public void close() throws IOException {
		if (this.invalidLineCount > 0) {
			if (LOG.isWarnEnabled()) {
				LOG.warn("In file \"" + currentSplit.getPath() + "\" (split start: " + this.splitStart + ") "
		...
		if (this.commentCount > 0) {
			if (LOG.isInfoEnabled()) {
				LOG.info("In file \"" + currentSplit.getPath() + "\" (split start: " + this.splitStart + ") "
