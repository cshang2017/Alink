...
public class ContinuousFileMonitoringFunction<OUT>
	...
	public ContinuousFileMonitoringFunction(
		...
		this.fs = Preconditions.checkNotNull(format.getFileSystem(), "Unspecified FileSystem.");
	...
	public void run(SourceContext <TimestampedFileInputSplit> context) throws Exception {
		...
		FileSystem fileSystem = FileSystemUtils.getFlinkFileSystem(fs, p.toString());
	...
	private void monitorDirAndForwardSplits(FileSystem fs,
		...
		Map <Long, List <TimestampedFileInputSplit>> splitsSortedByModTime = getInputSplitsSortedByModTime(
			eligibleFiles);
